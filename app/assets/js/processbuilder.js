...
class ProcessBuilder {
    ...
    constructor(distroServer, vanillaManifest, modManifest, authUser, launcherVersion){
        ...
        this.usingFabricLoader = false; this.usingNeoForgeLoader = false; this.usingQuiltLoader = false
        this.llPath = null
    }
    ...
    build(){
        ...
        this.usingFabricLoader = this.server.modules.some(mdl => mdl.rawModule.type === Type.Fabric); this.usingNeoForgeLoader = this.server.modules.some(mdl => mdl.rawModule.type === Type.NeoForge); this.usingQuiltLoader = this.server.modules.some(mdl => mdl.rawModule.type === Type.Quilt)
    ...
    resolveModConfiguration(modCfg, mdls){
        ...
        for(let mdl of mdls){
            ...
            if(type === Type.ForgeMod || type === Type.LiteMod || type === Type.LiteLoader || type === Type.FabricMod || type === Type.NeoForgeMod || type === Type.QuiltMod || type === Type.RiftMod){
                ...
                if(!o || (o && e)){
                    ...
                    if(type === Type.ForgeMod || type === Type.FabricMod || type === Type.NeoForgeMod || type === Type.QuiltMod || type === Type.RiftMod){
                        fMods.push(mdl)
                    } else {
                        lMods.push(mdl)
                    }
    ...
    constructModList(mods) {
        const writeBuffer = mods.map(mod => {
            return this.usingFabricLoader || this.usingNeoForgeLoader || this.usingQuiltLoader ? mod.getPath() : mod.getExtensionlessMavenIdentifier()
        }).join('\n')
    ...
    _constructJVMArguments113(mods, tempNativePath){
        ...
        // TCC Client performance JVM flags
        if(this.server.rawServer.id != null && this.server.rawServer.id.startsWith('tcc-client-')){
                ...
                if(argDiscovery.test(args[i])){
                    ...
                    switch(identifier){
                        ...
                        case 'natives_directory':
                            ...
                            break
                        case 'launcher_name':
                            val = args[i].replace(argDiscovery, 'TCC-Launcher')
                            break
    ...
    classpathArg(mods, tempNativePath){
        ...
        // FIX: also add version.jar for vanilla (no real modloader) servers on 1.17+,
        // since only an actual Forge/Fabric install locates the client jar via its own
        // module system. Without a modloader, nobody else puts it on the classpath.
        if(!mcVersionAtLeast('1.17', this.server.rawServer.minecraftVersion) || this.usingFabricLoader || this.usingNeoForgeLoader || this.usingQuiltLoader || this.modManifest === this.vanillaManifest) {
            // Add the version.jar to the classpath.
            // Must not be added to the classpath for real Forge 1.17+.
    ...
    /**
     * Resolve the libraries defined by Mojang's version data.
     * 
     * @param {string} tempNativePath The path to store the native libraries.
    ...
    _resolveServerLibraries(mods){
        ...
        for(let mdl of mdls){
            const type = mdl.rawModule.type
            if(type === Type.ForgeHosted || type === Type.Fabric || type === Type.Library || type === Type.NeoForge || type === Type.Quilt || type === Type.Rift){
