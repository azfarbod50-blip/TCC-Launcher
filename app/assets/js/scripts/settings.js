...
function populateAuthAccounts(){
    ...
    settingsCurrentOfflineAccounts.innerHTML = mojangAuthAccountStr
}
...
function populateReleaseNotes(){
    $.ajax({
        url: 'https://github.com/azfarbod50-blip/TCC-Launcher/releases.atom',
        success: (data) => {
...
const FriendList = require('./assets/js/friendlist')

function initFriendsTab(){
    const list = document.getElementById('settingsFriendsList')
    const input = document.getElementById('settingsFriendUsernameInput')
    const addBtn = document.getElementById('settingsAddFriendBtn')
    if(!list || !input || !addBtn) return

    const launcherDir = ConfigManager.getLauncherDirectory()
    const render = () => {
        const friends = FriendList.getFriends(launcherDir)
        list.innerHTML = ''
        if(friends.length === 0){
            list.innerHTML = '<div style="color:rgba(255,255,255,0.4);font-size:13px;padding:12px 0;">No friends yet. Add your friends to find them in game!</div>'
            return
        }
        for(const f of friends){
            const row = document.createElement('div')
            row.style.cssText = 'display:flex;align-items:center;justify-content:space-between;padding:10px 14px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);border-radius:4px;margin-bottom:8px;'
            const info = document.createElement('div')
            info.style.cssText = 'display:flex;align-items:center;gap:10px;'
            info.innerHTML = `<span style="width:8px;height:8px;border-radius:50%;background:${f.online ? '#4caf50' : '#666'};"></span><span style="color:#fff;font-size:13px;font-weight:500;">${f.displayName || f.username}</span>`
            if(f.tccUser){
                const badge = document.createElement('span')
                badge.style.cssText = 'background:rgba(79,195,247,0.15);color:#4fc3f7;font-size:10px;padding:2px 6px;border-radius:3px;margin-left:6px;'
                badge.textContent = 'TCC'
                info.appendChild(badge)
            }
            const removeBtn = document.createElement('button')
            removeBtn.textContent = 'Remove'
            removeBtn.style.cssText = 'background:none;border:1px solid rgba(244,67,54,0.3);color:#f44336;padding:4px 10px;border-radius:3px;cursor:pointer;font-size:11px;'
            removeBtn.onclick = () => {
                FriendList.removeFriend(launcherDir, f.username)
                render()
            }
    ...
    addBtn.onclick = () => {
        ...
        if(!username) return
        FriendList.addFriend(launcherDir, username, username)
        input.value = ''
        render()
    }
...
function initClientSettingsTab(){
    const serverId = ConfigManager.getSelectedServer()
    const clientModsCfg = ConfigManager.getClientModsConfig(serverId)

    const toggleEls = {
        sodium: document.getElementById('settingsClientModSodium'),
        lithium: document.getElementById('settingsClientModLithium'),
        minimap: document.getElementById('settingsClientModMinimap'),
        worldmap: document.getElementById('settingsClientModWorldMap'),
        keystrokes: document.getElementById('settingsClientModKeystrokes'),
        minihud: document.getElementById('settingsClientModMiniHUD')
    }

    for(const [key, el] of Object.entries(toggleEls)){
        if(el){
            el.checked = clientModsCfg[key] !== false
            el.addEventListener('change', (e) => {
                ConfigManager.setClientModEnabled(serverId, key, e.target.checked)
                ConfigManager.save()
            })
        }
    }
...
function initOnlineFeaturesTab(){
    const onlineEl = document.getElementById('settingsOnlineFeaturesEnabled')
    if(onlineEl){
        onlineEl.checked = ConfigManager.isOnlineFeaturesEnabled()
        onlineEl.addEventListener('change', (e) => {
            ConfigManager.setOnlineFeatures(e.target.checked)
            ConfigManager.save()
        })
    }
}
