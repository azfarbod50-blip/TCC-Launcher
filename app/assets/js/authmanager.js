...
/**
 * Add an offline account. No credentials are validated with Mojang; a random
 * UUID and a placeholder access token are generated locally and stored the
 * same way a real Mojang account would be. Intended for use when the user
 * has no Mojang/Microsoft account (e.g. cracked/offline-mode servers).
 * 
 * @param {string} username The desired offline username.
 * @returns {Object} The resolved offline account object.
 */
exports.addOfflineAccount = function(username) {
    ...
    // Generate a random uuid to stand in for a real Mojang profile id.
    const uuid = randomUUID().replace(/-/g, '')
    const accessToken = 'offline_' + randomUUID().replace(/-/g, '')

    const ret = ConfigManager.addMojangAuthAccount(uuid, accessToken, username, username)
    ConfigManager.save()
    return ret
}
...
exports.removeMojangAccount = async function(uuid){
    try {
        ...
        // Offline accounts were never registered with Mojang; skip the
        // invalidate call and just remove them locally.
        if(authAcc != null && typeof authAcc.accessToken === 'string' && authAcc.accessToken.startsWith('offline_')) {
...
async function validateSelectedMojangAccount(){
    ...
    // Offline accounts have no real token to validate against Mojang.
    if(typeof current.accessToken === 'string' && current.accessToken.startsWith('offline_')) {
        return true
    }
